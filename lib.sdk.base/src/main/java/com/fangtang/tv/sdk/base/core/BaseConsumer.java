...
public abstract class BaseConsumer<T> implements Consumer<T> {
    ...
    int turnOnStatusFlag(@Status int status, @Status int flag) {
        return status | flag;
    }
     ...
     */
    public static @Status
    int turnOffStatusFlag(@Status int status, @Status int flag) {
        return status & ~flag;
    }
     ...
     */
    public static @Status
    int simpleStatusForIsLast(boolean isLast) {
        return isLast ? IS_LAST : NO_FLAGS;
    }

    @Override
    public synchronized void onNewResult(T newResult, @Status int status) {
        if (mIsFinished) {
            return;
        }
